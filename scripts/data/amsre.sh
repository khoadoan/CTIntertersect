...
#
# EXAMPLE:
# 	$ amsre.sh 2B-GEOPROF AE_L2A 002 12 1
#
# Author:	Khoa Doan
...
for ((DOM=2; DOM<3; DOM++)) ; do
	# Set destination directory.
	DSTDIR=`printf \
		'%s/AMSR-E/%s/%03d/%s' \
		"$CINTDATA" "$YEAR" "$DOM" "$RELEASE"`
    ...
    # Set source.
    SRCDIR=`printf \
    'ftp://n4ftl01u.ecs.nasa.gov/SAN/AMSA/%s.%s/%4d.%02d.%02d/*.hdf' \
    "$PROD" "$RELEASE" "$YEAR" "$MONTH" "$DOM"`
