function [ output_args ] = write_intersect_output( intersections, output_dir )
    ...
    for iIndex = 1:noOfIntersections
        ...
        % TODO: Dirty Trick, need to discuss with KUO
%         trmm_2A25_file = regexprep(trmm_2A25_file, '7\.HDF', '6\.HDF');
%         trmm_2A25_file = regexprep(trmm_2A25_file, '200707', '0707')
        ...
        cs_scan_context_range = max((pairs(1,1)-50), 1):min((pairs(cs_curtain_length,1)+50), cs_scan_length);
        if pairs(1,2) > pairs(cs_curtain_length,2)
            trmm_scan_block_range = min((pairs(1,2)+10), trmm_scan_length) :-1:max((pairs(cs_curtain_length,2)-10),1);
            trmm_curtain_range = pairs(1,2):-1:pairs(cs_curtain_length,2);
        else
            trmm_scan_block_range = max((pairs(1,2)-10), 1):1:min((pairs(cs_curtain_length,2)+10), trmm_scan_length);
            trmm_curtain_range = pairs(1,2):1:pairs(cs_curtain_length,2);
        end
