cs_trmm_intersections = intersections;
        ...
        for i = 1:length(intersections)
            if intersections(i).cs_trmm_intersection(7) < tmi_time(tmi_last_index)
                ...
                if intersections(i).cs_trmm_intersection(7) > tmi_time(1)
                    % Read Data From AMSR-E
                    [tmi_orbit_latitudes, tmi_orbit_longitudes, tmi_orbit_time] = read_tmi(tmi_directory, tmi_filenames(tmi_orbit_index).name, current_year, current_year);
